...
- Support for NodeJS versions 22 and 24   

### Breaking Changes 
- The field `usageRecordId` was removed from the `CreateSubscriptionUsageRecordDetails` model in the Service Enablement Lifecycle Framework service
...
### Breaking Changes 
- The type of the field `BillingDetails` was changed to `[]BillingDetails` in the model `SubscriptionDetails` in the Service Enablement Lifecycle Framework service  
- The type of the field `BillingFrequency` was changed to `BillingFrequencyEnum` in the model `PricingPlan` in the Service Enablement Lifecycle Framework service  
- The enum value `selfTokenId` was removed from the enum `ListingSubscriptionsSortByEnum` in the Service Enablement Lifecycle Framework service
...
### Breaking Changes 
- The field `compartmentId` was made required in the request model `CreatePrivilegedApiRequestDetails` and request classes `ListApiMetadataByEntityTypesRequest`, `ListApiMetadataRequest`, `ListPrivilegedApiControlsRequest`, `ListPrivilegedApiRequestsRequest`, `ListWorkRequestsRequest` in the Oracle API Access Control service
...
### Breaking Changes 
- The field `inboundAuthConfig` was made required in the request model `CreateHostedApplicationDetails` in the Generative AI service
