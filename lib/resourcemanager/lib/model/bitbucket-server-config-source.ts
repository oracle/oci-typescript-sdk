/**
 * Resource Manager API
 * Use the Resource Manager API to automate deployment and operations for all Oracle Cloud Infrastructure resources.
Using the infrastructure-as-code (IaC) model, the service is based on Terraform, an open source industry standard that lets DevOps engineers develop and deploy their infrastructure anywhere.
For more information, see
[the Resource Manager documentation](/iaas/Content/ResourceManager/home.htm).

 * OpenAPI spec version: 20180917
 * 
 *
 * NOTE: This class is auto generated by OracleSDKGenerator.
 * Do not edit the class manually.
 *
 * Copyright (c) 2020, 2022, Oracle and/or its affiliates.  All rights reserved.
 * This software is dual-licensed to you under the Universal Permissive License (UPL) 1.0 as shown at https://oss.oracle.com/licenses/upl or Apache License 2.0 as shown at http://www.apache.org/licenses/LICENSE-2.0. You may choose either license.
 */

import * as model from "../model";
import common = require("oci-common");

/**
 * Metadata about the Bitbucket Server configuration source.
 *
 */
export interface BitbucketServerConfigSource extends model.ConfigSource {
  /**
   * The [OCID](https://docs.cloud.oracle.com/iaas/Content/General/Concepts/identifiers.htm) of the Bitbucket Server configuration source.
   *
   */
  "configurationSourceProviderId": string;
  /**
   * The URL of the Bitbucket Server repository for the configuration source.
   */
  "repositoryUrl": string;
  /**
   * The name of the branch in the Bitbucket Server repository for the configuration source.
   */
  "branchName"?: string;
  /**
   * Unique identifier for a Bitbucket Server project.
   */
  "projectId"?: string;
  /**
   * Bitbucket Server repository identifier, usually identified as <repository>.git.
   */
  "repositoryId"?: string;
  /**
   * The clone URL of Bitbucket Server configuration source.
   */
  "cloneUrl"?: string;

  "configSourceType": string;
}

export namespace BitbucketServerConfigSource {
  export function getJsonObj(obj: BitbucketServerConfigSource, isParentJsonObj?: boolean): object {
    const jsonObj = {
      ...(isParentJsonObj
        ? obj
        : (model.ConfigSource.getJsonObj(obj) as BitbucketServerConfigSource)),
      ...{}
    };

    return jsonObj;
  }
  export const configSourceType = "BITBUCKET_SERVER_CONFIG_SOURCE";
  export function getDeserializedJsonObj(
    obj: BitbucketServerConfigSource,
    isParentJsonObj?: boolean
  ): object {
    const jsonObj = {
      ...(isParentJsonObj
        ? obj
        : (model.ConfigSource.getDeserializedJsonObj(obj) as BitbucketServerConfigSource)),
      ...{}
    };

    return jsonObj;
  }
}
