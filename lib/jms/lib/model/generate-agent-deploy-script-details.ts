/**
 * Java Management Service API
 * API for the Java Management Service. Use this API to view, create, and manage Fleets.
 * OpenAPI spec version: 20210610
 *
 *
 * NOTE: This class is auto generated by OracleSDKGenerator.
 * Do not edit the class manually.
 *
 * Copyright (c) 2020, 2022, Oracle and/or its affiliates.  All rights reserved.
 * This software is dual-licensed to you under the Universal Permissive License (UPL) 1.0 as shown at https://oss.oracle.com/licenses/upl or Apache License 2.0 as shown at http://www.apache.org/licenses/LICENSE-2.0. You may choose either license.
 */

import * as model from "../model";
import common = require("oci-common");

/**
 * Attributes to generate agent deploy script for a Fleet.
 */
export interface GenerateAgentDeployScriptDetails {
  /**
   * The [OCID](https://docs.cloud.oracle.com/Content/General/Concepts/identifiers.htm) of the install key for which to generate the script.
   */
  "installKeyId": string;
  /**
   * The operating system type for the script. Currently only 'LINUX' and 'WINDOWS' are supported.
   */
  "osFamily": model.OsFamily;
  /**
   * Enable/disable user name collection on agent.
   */
  "isUserNameEnabled": boolean;
}

export namespace GenerateAgentDeployScriptDetails {
  export function getJsonObj(obj: GenerateAgentDeployScriptDetails): object {
    const jsonObj = { ...obj, ...{} };

    return jsonObj;
  }
  export function getDeserializedJsonObj(obj: GenerateAgentDeployScriptDetails): object {
    const jsonObj = { ...obj, ...{} };

    return jsonObj;
  }
}
