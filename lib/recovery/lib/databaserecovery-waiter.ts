...
export class DatabaseRecoveryWaiter {
  ...
  /**
   * Waits forLongTermBackup till it reaches any of the provided states
   *
   * @param request the request to send
   * @param targetStates the desired states to wait for. The waiter will return once the resource reaches any of the provided states
   * @return response returns GetLongTermBackupResponse | null (null in case of 404 response)
   */
  public async forLongTermBackup(
    request: serviceRequests.GetLongTermBackupRequest,
    ...targetStates: models.LongTermBackup.LifecycleState[]
  ): Promise<serviceResponses.GetLongTermBackupResponse | null> {
    return genericTerminalConditionWaiter(
      this.config,
      () => this.client.getLongTermBackup(request),
      response => targetStates.includes(response.longTermBackup.lifecycleState!),
      targetStates.includes(models.LongTermBackup.LifecycleState.Deleted)
    );
  }
