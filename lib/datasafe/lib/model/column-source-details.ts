/**
 * Data Safe API
 * APIs for using Oracle Data Safe.
 * OpenAPI spec version: 20181201
 *
 *
 * NOTE: This class is auto generated by OracleSDKGenerator.
 * Do not edit the class manually.
 *
 * Copyright (c) 2020, 2022, Oracle and/or its affiliates.  All rights reserved.
 * This software is dual-licensed to you under the Universal Permissive License (UPL) 1.0 as shown at https://oss.oracle.com/licenses/upl or Apache License 2.0 as shown at http://www.apache.org/licenses/LICENSE-2.0. You may choose either license.
 */

import * as model from "../model";
import common = require("oci-common");

/**
 * The source of masking columns.
 */
export interface ColumnSourceDetails {
  "columnSource": string;
}

export namespace ColumnSourceDetails {
  export function getJsonObj(obj: ColumnSourceDetails): object {
    const jsonObj = { ...obj, ...{} };

    if ("columnSource" in obj && obj.columnSource) {
      switch (obj.columnSource) {
        case "SENSITIVE_DATA_MODEL":
          return model.ColumnSourceFromSdmDetails.getJsonObj(
            <model.ColumnSourceFromSdmDetails>(<object>jsonObj),
            true
          );
        case "TARGET":
          return model.ColumnSourceFromTargetDetails.getJsonObj(
            <model.ColumnSourceFromTargetDetails>(<object>jsonObj),
            true
          );
        default:
          throw Error("Unknown value for: " + obj.columnSource);
      }
    }
    return jsonObj;
  }
  export function getDeserializedJsonObj(obj: ColumnSourceDetails): object {
    const jsonObj = { ...obj, ...{} };

    if ("columnSource" in obj && obj.columnSource) {
      switch (obj.columnSource) {
        case "SENSITIVE_DATA_MODEL":
          return model.ColumnSourceFromSdmDetails.getDeserializedJsonObj(
            <model.ColumnSourceFromSdmDetails>(<object>jsonObj),
            true
          );
        case "TARGET":
          return model.ColumnSourceFromTargetDetails.getDeserializedJsonObj(
            <model.ColumnSourceFromTargetDetails>(<object>jsonObj),
            true
          );
        default:
          throw Error("Unknown value for: " + obj.columnSource);
      }
    }
    return jsonObj;
  }
}
