...
export class DataScienceWaiter {
  ...
  /**
   * Waits forComputeTarget till it reaches any of the provided states
   *
   * @param request the request to send
   * @param targetStates the desired states to wait for. The waiter will return once the resource reaches any of the provided states
   * @return response returns GetComputeTargetResponse | null (null in case of 404 response)
   */
  public async forComputeTarget(
    request: serviceRequests.GetComputeTargetRequest,
    ...targetStates: models.ComputeTargetLifecycleState[]
  ): Promise<serviceResponses.GetComputeTargetResponse | null> {
    return genericTerminalConditionWaiter(
      this.config,
      () => this.client.getComputeTarget(request),
      response => targetStates.includes(response.computeTarget.lifecycleState!),
      targetStates.includes(models.ComputeTargetLifecycleState.Deleted)
    );
  }
