/**
 * MarketplacePrivateOffer API
 * Use the Marketplace Publisher API to manage the publishing of applications in Oracle Cloud Infrastructure Marketplace.
 * OpenAPI spec version: 20220901
 *
 *
 * NOTE: This class is auto generated by OracleSDKGenerator.
 * Do not edit the class manually.
 *
 * Copyright (c) 2020, 2026, Oracle and/or its affiliates.  All rights reserved.
 * This software is dual-licensed to you under the Universal Permissive License (UPL) 1.0 as shown at https://oss.oracle.com/licenses/upl or Apache License 2.0 as shown at http://www.apache.org/licenses/LICENSE-2.0. You may choose either license.
 */

import * as model from "../model";
import common = require("oci-common");

/**
 * Description of an offer quote attachment.
 */
export interface QuoteAttachment {
  /**
   * Unique identifier that is immutable on creation.
   */
  "id": string;
  /**
   * Unique identifier of the associated offer quote that is immutable on creation.
   */
  "offerQuoteId": string;
  /**
   * OCID of the reseller's tenancy (root compartment).
   */
  "resellerCompartmentId": string;
  /**
   * OCID of the ISV's tenancy (root compartment).
   */
  "isvCompartmentId": string;
  /**
   * The name used to refer to the uploaded data.
   */
  "displayName": string;
  /**
   * The type of offer quote attachment.
   */
  "type": QuoteAttachment.Type;
  /**
   * The time the offer quote attachment was created. An RFC3339 formatted datetime string.
   */
  "timeCreated": Date;
  /**
   * The time the offer quote attachment was updated. An RFC3339 formatted datetime string.
   */
  "timeUpdated"?: Date;
  /**
   * The current state of the offer quote attachment.
   */
  "lifecycleState": QuoteAttachment.LifecycleState;
  /**
   * Provides detailed information about the current state of the offer quote attachment. For example, if the offer quote attachment is in a failed state this message can include specific validation errors.
   */
  "lifecycleDetails"?: string;
  /**
   * The MIME type of the uploaded data.
   */
  "mimeType"?: string;
  /**
   * The recipient of the offer quote attachment.
   */
  "recipient": QuoteAttachment.Recipient;
  /**
   * Simple key-value pair that is applied without any predefined name, type or scope. Exists for cross-compatibility only.
   * Example: {@code {\"bar-key\": \"value\"}}
   *
   */
  "freeformTags"?: { [key: string]: string };
  /**
   * Defined tags for this resource. Each key is predefined and scoped to a namespace.
   * Example: {@code {\"foo-namespace\": {\"bar-key\": \"value\"}}}
   *
   */
  "definedTags"?: { [key: string]: { [key: string]: any } };
  /**
   * System tags for this resource. Each key is predefined and scoped to a namespace.
   * Example: {@code {\"orcl-cloud\": {\"free-tier-retained\": \"true\"}}}
   *
   */
  "systemTags"?: { [key: string]: { [key: string]: any } };
}

export namespace QuoteAttachment {
  export enum Type {
    ContractTAndC = "CONTRACT_T_AND_C",
    Quote = "QUOTE",
    Eula = "EULA",
    TermsOfUse = "TERMS_OF_USE",
    Misc = "MISC",
    /**
     * This value is used if a service returns a value for this enum that is not recognized by this
     * version of the SDK.
     */
    UnknownValue = "UNKNOWN_VALUE"
  }

  export enum LifecycleState {
    Creating = "CREATING",
    Updating = "UPDATING",
    Active = "ACTIVE",
    Deleting = "DELETING",
    Deleted = "DELETED",
    Failed = "FAILED",
    /**
     * This value is used if a service returns a value for this enum that is not recognized by this
     * version of the SDK.
     */
    UnknownValue = "UNKNOWN_VALUE"
  }

  export enum Recipient {
    Reseller = "RESELLER",
    Buyer = "BUYER",
    /**
     * This value is used if a service returns a value for this enum that is not recognized by this
     * version of the SDK.
     */
    UnknownValue = "UNKNOWN_VALUE"
  }

  export function getJsonObj(obj: QuoteAttachment): object {
    const jsonObj = { ...obj, ...{} };

    return jsonObj;
  }
  export function getDeserializedJsonObj(obj: QuoteAttachment): object {
    const jsonObj = { ...obj, ...{} };

    return jsonObj;
  }
}
