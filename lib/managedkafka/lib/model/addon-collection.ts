...
export interface AddonCollection {
   ...
   */
  "items": Array<model.AddonSummary>;
}

export namespace AddonCollection {
  export function getJsonObj(obj: AddonCollection): object {
    const jsonObj = {
      ...obj,
      ...{
        "items": obj.items
          ? obj.items.map(item => {
              return model.AddonSummary.getJsonObj(item);
            })
          : undefined
      }
    };
  ...
  export function getDeserializedJsonObj(obj: AddonCollection): object {
    const jsonObj = {
      ...obj,
      ...{
        "items": obj.items
          ? obj.items.map(item => {
              return model.AddonSummary.getDeserializedJsonObj(item);
            })
          : undefined
      }
    };
