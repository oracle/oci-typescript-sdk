/**
 * Batch API
 * Use the Batch Control Plane API to encapsulate and manage all aspects of computationally intensive jobs.
 * OpenAPI spec version: 20251031
 *
 *
 * NOTE: This class is auto generated by OracleSDKGenerator.
 * Do not edit the class manually.
 *
 * Copyright (c) 2020, 2026, Oracle and/or its affiliates.  All rights reserved.
 * This software is dual-licensed to you under the Universal Permissive License (UPL) 1.0 as shown at https://oss.oracle.com/licenses/upl or Apache License 2.0 as shown at http://www.apache.org/licenses/LICENSE-2.0. You may choose either license.
 */

import * as model from "../model";
import common = require("oci-common");

/**
 * The data to update logging configuration for OCI Logging service.
 */
export interface UpdateOciLoggingConfiguration extends model.UpdateLoggingConfigurationDetails {
  /**
   * A switch to enable or disable propagation of job and task events to the customer's logs in OCI logging service.
   */
  "isJobTaskEventsPropagationEnabled": boolean;

  "type": string;
}

export namespace UpdateOciLoggingConfiguration {
  export function getJsonObj(
    obj: UpdateOciLoggingConfiguration,
    isParentJsonObj?: boolean
  ): object {
    const jsonObj = {
      ...(isParentJsonObj
        ? obj
        : (model.UpdateLoggingConfigurationDetails.getJsonObj(
            obj
          ) as UpdateOciLoggingConfiguration)),
      ...{}
    };

    return jsonObj;
  }
  export const type = "OCI_LOGGING";
  export function getDeserializedJsonObj(
    obj: UpdateOciLoggingConfiguration,
    isParentJsonObj?: boolean
  ): object {
    const jsonObj = {
      ...(isParentJsonObj
        ? obj
        : (model.UpdateLoggingConfigurationDetails.getDeserializedJsonObj(
            obj
          ) as UpdateOciLoggingConfiguration)),
      ...{}
    };

    return jsonObj;
  }
}
