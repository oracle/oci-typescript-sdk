/**
 * Oracle Integration API
 * Oracle Integration API.
 * OpenAPI spec version: 20190131
 *
 *
 * NOTE: This class is auto generated by OracleSDKGenerator.
 * Do not edit the class manually.
 *
 * Copyright (c) 2020, 2022, Oracle and/or its affiliates.  All rights reserved.
 * This software is dual-licensed to you under the Universal Permissive License (UPL) 1.0 as shown at https://oss.oracle.com/licenses/upl or Apache License 2.0 as shown at http://www.apache.org/licenses/LICENSE-2.0. You may choose either license.
 */

import * as model from "../model";
import common = require("oci-common");

/**
 * Virtual Cloud Network definition.
 *
 */
export interface VirtualCloudNetwork {
  /**
   * The Virtual Cloud Network OCID.
   *
   */
  "id": string;
  /**
   * Source IP addresses or IP address ranges ingress rules. (ex: \"168.122.59.5\", \"10.20.30.0/26\")
   * An invalid IP or CIDR block will result in a 400 response.
   *
   */
  "allowlistedIps"?: Array<string>;
}

export namespace VirtualCloudNetwork {
  export function getJsonObj(obj: VirtualCloudNetwork): object {
    const jsonObj = { ...obj, ...{} };

    return jsonObj;
  }
  export function getDeserializedJsonObj(obj: VirtualCloudNetwork): object {
    const jsonObj = { ...obj, ...{} };

    return jsonObj;
  }
}
