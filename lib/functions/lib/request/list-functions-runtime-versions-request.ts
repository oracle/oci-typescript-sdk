/**
 *
 *
 * OpenAPI spec version: 20260325
 *
 *
 * NOTE: This class is auto generated by OracleSDKGenerator.
 * Do not edit the class manually.
 *
 * Copyright (c) 2020, 2026, Oracle and/or its affiliates.  All rights reserved.
 * This software is dual-licensed to you under the Universal Permissive License (UPL) 1.0 as shown at https://oss.oracle.com/licenses/upl or Apache License 2.0 as shown at http://www.apache.org/licenses/LICENSE-2.0. You may choose either license.
 */

import * as model from "../model";
import common = require("oci-common");

/**
 * @example Click {@link https://docs.oracle.com/en-us/iaas/tools/typescript-sdk-examples/latest/functions/ListFunctionsRuntimeVersions.ts.html |here} to see how to use ListFunctionsRuntimeVersionsRequest.
 */
export interface ListFunctionsRuntimeVersionsRequest extends common.BaseRequest {
  /**
   * unique FunctionsRuntime identifier
   */
  "functionsRuntimeId"?: string;
  /**
   * A filter to return only resources that match the entire FunctionsRuntime name given.
   */
  "functionsRuntimeName"?: string;
  /**
   * unique FunctionsRuntimeVersion identifier
   */
  "functionsRuntimeVersionId"?: string;
  /**
   * A filter to return only resources that match the entire FunctionsRuntimeVersion name given.
   */
  "displayName"?: string;
  /**
   * A filter to return only resources that match the entire osVersion name given.
   */
  "osVersion"?: string;
  /**
   * A filter to return only resources that match the entire languageVersion name given.
   */
  "languageVersion"?: string;
  /**
   * Matches the current version associated with a FunctionsRuntime.
   *
   */
  "isCurrentVersion"?: boolean;
  /**
   * A filter to return only resources their lifecycleState matches the given lifecycleState.
   */
  "lifecycleState"?: string;
  /**
   * The field to sort by. Only one sort order may be provided. Default order for timeCreated is descending. Default order for name is ascending.
   *
   */
  "sortBy"?: ListFunctionsRuntimeVersionsRequest.SortBy;
  /**
 * The maximum number of items to return. 1 is the minimum, 50 is the maximum.
* <p>
Default: 10
* 
 */
  "limit"?: number;
  /**
   * The pagination token for a list query returned by a previous operation
   *
   */
  "page"?: string;
  /**
   * Specifies sort order.
   * <p>
   * **ASC:** Ascending sort order.
   * * **DESC:** Descending sort order.
   *
   */
  "sortOrder"?: ListFunctionsRuntimeVersionsRequest.SortOrder;
  /**
   * The unique Oracle-assigned identifier for the request. If you need to contact Oracle about a
   * particular request, please provide the request ID.
   *
   */
  "opcRequestId"?: string;
}

export namespace ListFunctionsRuntimeVersionsRequest {
  export enum SortBy {
    TimeCreated = "timeCreated",
    Name = "name"
  }

  export enum SortOrder {
    Asc = "ASC",
    Desc = "DESC"
  }
}
