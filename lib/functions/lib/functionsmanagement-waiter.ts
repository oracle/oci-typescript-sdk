...
export class FunctionsManagementWaiter {
  ...
  /**
   * Waits forFunctionsRuntime till it reaches any of the provided states
   *
   * @param request the request to send
   * @param targetStates the desired states to wait for. The waiter will return once the resource reaches any of the provided states
   * @return response returns GetFunctionsRuntimeResponse
   */
  public async forFunctionsRuntime(
    request: serviceRequests.GetFunctionsRuntimeRequest,
    ...targetStates: models.FunctionsRuntime.LifecycleState[]
  ): Promise<serviceResponses.GetFunctionsRuntimeResponse> {
    return genericWaiter(
      this.config,
      () => this.client.getFunctionsRuntime(request),
      response => targetStates.includes(response.functionsRuntime.lifecycleState!)
    );
  }

  /**
   * Waits forFunctionsRuntimeVersion till it reaches any of the provided states
   *
   * @param request the request to send
   * @param targetStates the desired states to wait for. The waiter will return once the resource reaches any of the provided states
   * @return response returns GetFunctionsRuntimeVersionResponse
   */
  public async forFunctionsRuntimeVersion(
    request: serviceRequests.GetFunctionsRuntimeVersionRequest,
    ...targetStates: models.FunctionsRuntimeVersion.LifecycleState[]
  ): Promise<serviceResponses.GetFunctionsRuntimeVersionResponse> {
    return genericWaiter(
      this.config,
      () => this.client.getFunctionsRuntimeVersion(request),
      response => targetStates.includes(response.functionsRuntimeVersion.lifecycleState!)
    );
  }
