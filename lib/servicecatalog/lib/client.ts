...
export class ServiceCatalogClient {
  ...
  public async getConfiguration(
    ...
    if (this.logger) this.logger.debug("Calling operation ServiceCatalogClient#getConfiguration.");
  ...
  public async listAllApplications(
    listAllApplicationsRequest: requests.ListAllApplicationsRequest
  ): Promise<responses.ListAllApplicationsResponse> {
    if (this.logger)
    ...
    const retrier = GenericRetrier.createPreferredRetrier(
      this._clientConfiguration ? this._clientConfiguration.retryConfiguration : undefined,
      listAllApplicationsRequest.retryConfiguration,
      specRetryConfiguration
    );
    if (this.logger) retrier.logger = this.logger;
    const request = await composeRequest({
    ...
    try {
      ...
      const sdkResponse = composeResponse({
        responseObject: <responses.ListAllApplicationsResponse>{},
        body: await response.json(),
