...
export class DbMulticloudAwsProviderWaiter {
  public constructor(
    private client: DbMulticloudAwsProviderClient,
   ...
   */
  public async forOracleDbAwsIdentityConnector(
    request: serviceRequests.GetOracleDbAwsIdentityConnectorRequest,
    ...targetStates: models.OracleDbAwsIdentityConnector.LifecycleState[]
  ): Promise<serviceResponses.GetOracleDbAwsIdentityConnectorResponse | null> {
    return genericTerminalConditionWaiter(
      this.config,
      () => this.client.getOracleDbAwsIdentityConnector(request),
      response => targetStates.includes(response.oracleDbAwsIdentityConnector.lifecycleState!),
      targetStates.includes(models.OracleDbAwsIdentityConnector.LifecycleState.Deleted)
    );
  }
   ...
   */
  public async forOracleDbAwsKey(
    request: serviceRequests.GetOracleDbAwsKeyRequest,
    ...targetStates: models.OracleDbAwsKey.LifecycleState[]
  ): Promise<serviceResponses.GetOracleDbAwsKeyResponse | null> {
    return genericTerminalConditionWaiter(
      this.config,
      () => this.client.getOracleDbAwsKey(request),
      response => targetStates.includes(response.oracleDbAwsKey.lifecycleState!),
      targetStates.includes(models.OracleDbAwsKey.LifecycleState.Deleted)
    );
  }
}
