/**
 * Network Firewall API
 * Use the Network Firewall API to create network firewalls and configure policies that regulates network traffic in and across VCNs.
 * OpenAPI spec version: 20230501
 *
 *
 * NOTE: This class is auto generated by OracleSDKGenerator.
 * Do not edit the class manually.
 *
 * Copyright (c) 2020, 2026, Oracle and/or its affiliates.  All rights reserved.
 * This software is dual-licensed to you under the Universal Permissive License (UPL) 1.0 as shown at https://oss.oracle.com/licenses/upl or Apache License 2.0 as shown at http://www.apache.org/licenses/LICENSE-2.0. You may choose either license.
 */

import * as model from "../model";
import common = require("oci-common");

/**
 * Request for creating NATV4 type Nat Rule used in the firewall policy.
 *
 */
export interface CreateNatV4RuleDetails extends model.CreateNatRuleDetails {
  "condition": model.NatRuleMatchCriteria;
  /**
   * action:
   * <p>
   * DIPP_SRC_NAT - Dynamic-ip-port source NAT.
   *
   */
  "action": model.NatV4ActionType;

  "type": string;
}

export namespace CreateNatV4RuleDetails {
  export function getJsonObj(obj: CreateNatV4RuleDetails, isParentJsonObj?: boolean): object {
    const jsonObj = {
      ...(isParentJsonObj
        ? obj
        : (model.CreateNatRuleDetails.getJsonObj(obj) as CreateNatV4RuleDetails)),
      ...{
        "condition": obj.condition
          ? model.NatRuleMatchCriteria.getJsonObj(obj.condition)
          : undefined
      }
    };

    return jsonObj;
  }
  export const type = "NATV4";
  export function getDeserializedJsonObj(
    obj: CreateNatV4RuleDetails,
    isParentJsonObj?: boolean
  ): object {
    const jsonObj = {
      ...(isParentJsonObj
        ? obj
        : (model.CreateNatRuleDetails.getDeserializedJsonObj(obj) as CreateNatV4RuleDetails)),
      ...{
        "condition": obj.condition
          ? model.NatRuleMatchCriteria.getDeserializedJsonObj(obj.condition)
          : undefined
      }
    };

    return jsonObj;
  }
}
