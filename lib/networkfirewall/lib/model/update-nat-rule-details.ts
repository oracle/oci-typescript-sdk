/**
 * Network Firewall API
 * Use the Network Firewall API to create network firewalls and configure policies that regulates network traffic in and across VCNs.
 * OpenAPI spec version: 20230501
 *
 *
 * NOTE: This class is auto generated by OracleSDKGenerator.
 * Do not edit the class manually.
 *
 * Copyright (c) 2020, 2026, Oracle and/or its affiliates.  All rights reserved.
 * This software is dual-licensed to you under the Universal Permissive License (UPL) 1.0 as shown at https://oss.oracle.com/licenses/upl or Apache License 2.0 as shown at http://www.apache.org/licenses/LICENSE-2.0. You may choose either license.
 */

import * as model from "../model";
import common = require("oci-common");

/**
 * Request for updating NAT Rule used in the firewall policy.
 * A Nat Rule is used to define to which traffic NAT should be applied by the firewall, and how it should do so.
 *
 */
export interface UpdateNatRuleDetails {
  /**
   * Description of a NAT rule. This field can be used to add additional info.
   */
  "description"?: string;
  "position"?: model.RulePosition;

  "type": string;
}

export namespace UpdateNatRuleDetails {
  export function getJsonObj(obj: UpdateNatRuleDetails): object {
    const jsonObj = {
      ...obj,
      ...{
        "position": obj.position ? model.RulePosition.getJsonObj(obj.position) : undefined
      }
    };

    if (obj && "type" in obj && obj.type) {
      switch (obj.type) {
        case "NATV4":
          return model.UpdateNatV4RuleDetails.getJsonObj(
            <model.UpdateNatV4RuleDetails>(<object>jsonObj),
            true
          );
        default:
          if (common.LOG.logger) common.LOG.logger.info(`Unknown value for: ${obj.type}`);
      }
    }
    return jsonObj;
  }
  export function getDeserializedJsonObj(obj: UpdateNatRuleDetails): object {
    const jsonObj = {
      ...obj,
      ...{
        "position": obj.position
          ? model.RulePosition.getDeserializedJsonObj(obj.position)
          : undefined
      }
    };

    if (obj && "type" in obj && obj.type) {
      switch (obj.type) {
        case "NATV4":
          return model.UpdateNatV4RuleDetails.getDeserializedJsonObj(
            <model.UpdateNatV4RuleDetails>(<object>jsonObj),
            true
          );
        default:
          if (common.LOG.logger) common.LOG.logger.info(`Unknown value for: ${obj.type}`);
      }
    }
    return jsonObj;
  }
}
