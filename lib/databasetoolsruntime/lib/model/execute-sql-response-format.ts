/**
 * Database Tools Runtime API
 * Use the Database Tools Runtime API to connect to databases through Database Tools Connections.
 * OpenAPI spec version: 20230222
 *
 *
 * NOTE: This class is auto generated by OracleSDKGenerator.
 * Do not edit the class manually.
 *
 * Copyright (c) 2020, 2026, Oracle and/or its affiliates.  All rights reserved.
 * This software is dual-licensed to you under the Universal Permissive License (UPL) 1.0 as shown at https://oss.oracle.com/licenses/upl or Apache License 2.0 as shown at http://www.apache.org/licenses/LICENSE-2.0. You may choose either license.
 */

import * as model from "../model";
import common = require("oci-common");

/**
 * Specifies which major elements of the response are returned
 */
export interface ExecuteSqlResponseFormat {
  /**
   * Set to false to exclude result set metadata from response
   */
  "resultSetMetaData"?: boolean;
  /**
   * Set to false to exclude statement information from response
   */
  "statementInformation"?: boolean;
  /**
   * Set to false to exclude statement text from response
   */
  "statementText"?: boolean;
  /**
   * Set to false to exclude binds from response
   */
  "binds"?: boolean;
  /**
   * Set to false to exclude result from response
   */
  "result"?: boolean;
  /**
   * Set to false to exclude response from response
   */
  "response"?: boolean;
}

export namespace ExecuteSqlResponseFormat {
  export function getJsonObj(obj: ExecuteSqlResponseFormat): object {
    const jsonObj = { ...obj, ...{} };

    return jsonObj;
  }
  export function getDeserializedJsonObj(obj: ExecuteSqlResponseFormat): object {
    const jsonObj = { ...obj, ...{} };

    return jsonObj;
  }
}
