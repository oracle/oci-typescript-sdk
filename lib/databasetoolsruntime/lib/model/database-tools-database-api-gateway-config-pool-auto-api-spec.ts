/**
 * Database Tools Runtime API
 * Use the Database Tools Runtime API to connect to databases through Database Tools Connections.
 * OpenAPI spec version: 20230222
 *
 *
 * NOTE: This class is auto generated by OracleSDKGenerator.
 * Do not edit the class manually.
 *
 * Copyright (c) 2020, 2026, Oracle and/or its affiliates.  All rights reserved.
 * This software is dual-licensed to you under the Universal Permissive License (UPL) 1.0 as shown at https://oss.oracle.com/licenses/upl or Apache License 2.0 as shown at http://www.apache.org/licenses/LICENSE-2.0. You may choose either license.
 */

import * as model from "../model";
import common = require("oci-common");

/**
 * The content of a Database Tools database API gateway config auto API spec sub resource defined within a pool.
 */
export interface DatabaseToolsDatabaseApiGatewayConfigPoolAutoApiSpec {
  /**
   * A system generated string that uniquely identifies an auto API spec sub resource within a given pool.
   */
  "key": string;
  /**
   * A user-friendly name. Does not have to be unique, and it\u2019s changeable. Avoid entering confidential information.
   */
  "displayName": string;
  /**
   * The name of the database object.
   */
  "databaseObjectName"?: string;
  /**
   * The type of the database object.
   */
  "databaseObjectType"?: DatabaseToolsDatabaseApiGatewayConfigPoolAutoApiSpec.DatabaseObjectType;
  /**
   * Description of the autoApiSpec.
   */
  "description"?: string;
  /**
   * Used as the URI path element for this object. When not specified the objectName lowercase is the default value.
   */
  "alias"?: string;
  /**
   * The operations to limit access to this resource. If not specified then the default is [\"READ\",\"WRITE\"].
   */
  "operations"?: Array<DatabaseToolsDatabaseApiGatewayConfigPoolAutoApiSpec.Operations>;
  /**
   * The security schemes that can access this resource. If not specified then the resource is public.
   */
  "securitySchemes"?: Array<DatabaseToolsDatabaseApiGatewayConfigPoolAutoApiSpec.SecuritySchemes>;
  /**
   * The name of the database API gateway config privilege protecting the resource. Only valid for SCOPE JWT Profile pools and BEARER securitySchemes.
   */
  "scope"?: string;
  /**
   * The name of the database API gateway config roles protecting the resource. Only valid for RBAC JWT Profile pools and BEARER securitySchemes.
   */
  "roles"?: Array<string>;
  /**
   * The time the resource was created. An RFC3339 formatted datetime string.
   */
  "timeCreated": Date;
  /**
   * The time the resource was updated. An RFC3339 formatted datetime string.
   */
  "timeUpdated": Date;

  "type": string;
}

export namespace DatabaseToolsDatabaseApiGatewayConfigPoolAutoApiSpec {
  export enum DatabaseObjectType {
    Function = "FUNCTION",
    Mview = "MVIEW",
    Package = "PACKAGE",
    Procedure = "PROCEDURE",
    Table = "TABLE",
    View = "VIEW",
    Dualityview = "DUALITYVIEW",
    /**
     * This value is used if a service returns a value for this enum that is not recognized by this
     * version of the SDK.
     */
    UnknownValue = "UNKNOWN_VALUE"
  }

  export enum Operations {
    Read = "READ",
    Write = "WRITE",
    /**
     * This value is used if a service returns a value for this enum that is not recognized by this
     * version of the SDK.
     */
    UnknownValue = "UNKNOWN_VALUE"
  }

  export enum SecuritySchemes {
    Basic = "BASIC",
    Bearer = "BEARER",
    /**
     * This value is used if a service returns a value for this enum that is not recognized by this
     * version of the SDK.
     */
    UnknownValue = "UNKNOWN_VALUE"
  }

  export function getJsonObj(obj: DatabaseToolsDatabaseApiGatewayConfigPoolAutoApiSpec): object {
    const jsonObj = { ...obj, ...{} };

    if (obj && "type" in obj && obj.type) {
      switch (obj.type) {
        case "DEFAULT":
          return model.DatabaseToolsDatabaseApiGatewayConfigPoolAutoApiSpecDefault.getJsonObj(
            <model.DatabaseToolsDatabaseApiGatewayConfigPoolAutoApiSpecDefault>(<object>jsonObj),
            true
          );
        default:
          if (common.LOG.logger) common.LOG.logger.info(`Unknown value for: ${obj.type}`);
      }
    }
    return jsonObj;
  }
  export function getDeserializedJsonObj(
    obj: DatabaseToolsDatabaseApiGatewayConfigPoolAutoApiSpec
  ): object {
    const jsonObj = { ...obj, ...{} };

    if (obj && "type" in obj && obj.type) {
      switch (obj.type) {
        case "DEFAULT":
          return model.DatabaseToolsDatabaseApiGatewayConfigPoolAutoApiSpecDefault.getDeserializedJsonObj(
            <model.DatabaseToolsDatabaseApiGatewayConfigPoolAutoApiSpecDefault>(<object>jsonObj),
            true
          );
        default:
          if (common.LOG.logger) common.LOG.logger.info(`Unknown value for: ${obj.type}`);
      }
    }
    return jsonObj;
  }
}
